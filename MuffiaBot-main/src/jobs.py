...
async def finish_registration(context):
    ...
    context.job_queue.run_once(night, 
                               ...
                               name=str(chat_id) + '_Наступає Ніч',
                               chat_id=chat_id)
...
async def night(context):
    ...
    if chats[chat_id].nights_passed > 1:
        ...
        summary_message = 'День голосування закінчився'
        if victim is not None:
            summary_message += f'\n{victim.markdown_link} був лінчуваний, його *{victim.role}*'
        ...
        summary_message += '\nГравці, що залишилися:' + chats[chat_id].alive_players_links()
        ...
        game_ended = await helpers.check_game_finish(chat_id, context, 'Наступний_день')
        if game_ended:
            return
    
    await context.bot.send_message(chat_id=chat_id, 
                                   text=f'*Ніч {chats[chat_id].nights_passed}* Починається',
...
async def day(context):
    ...
    day_message = f'*День {chats[chat_id].nights_passed}* Починається'
    victims = chats[chat_id].get_night_victims()
    for victim in victims:
        day_message += f'\n{victim.markdown_link} був убитий вночі, його *{victim.role}*'
    chats[chat_id].handle_victims(victims)

    if len(victims) == 0:
        day_message += '\nНікого не вбили вночі'
    
    await context.bot.send_message(chat_id=chat_id, text=day_message, parse_mode='MarkdownV2')
    if helpers.check_game_stopped(chat_id, game_id):
        return

    game_ended = await helpers.check_game_finish(chat_id, context, 'Наступна_ніч')
    if game_ended:
        return
