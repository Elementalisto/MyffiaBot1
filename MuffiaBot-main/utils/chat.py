...
class Chat:
    def __init__(self, id, bot_username):
        ...
        self.registration_keyboard = InlineKeyboardMarkup.from_button(InlineKeyboardButton(text='Реєстрація',
                                                                                           url=url))
    ...
    def build_detective_action_keyboard(self):
        keyboard = [[InlineKeyboardButton('Вбити', callback_data=f'{self.query_header}_detkill_{self.nights_passed}_'),
                     InlineKeyboardButton('Перевірити', callback_data=f'{self.query_header}_detcheck_{self.nights_passed}_')]]
        return InlineKeyboardMarkup(keyboard)
    ...
    def build_doctor_keyboard(self):
        ...
        keyboard.append([InlineKeyboardButton('Себе',
    ...
    def build_general_keyboard(self, id):
        ...
        keyboard.append([InlineKeyboardButton('Пропустити голосування',
    ...
    def check_game_ended(self, when):
        ...
        if self.detective and self.doctor:
            if when == 'Наступний_День':
                lower_bound += 2
            else:
                lower_bound += 1
        elif self.detective and not self.doctor:
            lower_bound += 1
        elif not self.detective and not self.doctor and when == 'Наступний_День':
            lower_bound -= 1
